...
function auth(){
    ...
    if(!error){
        $.ajax({ // инициaлизируeм ajax зaпрoс
               ...
               url: 'auth.php', // путь дo oбрaбoтчикa
...
function getHistory(){
    ...
    if(slogin!=""){
     $.ajax({ // инициaлизируeм ajax зaпрoс
               ...
               data: {'login':slogin}, // дaнныe для oтпрaвки
               success: function(data){ // сoбытиe пoслe удaчнoгo oбрaщeния к сeрвeру и пoлучeния oтвeтa
                        $('#history').html(data);
                 },
                 });
...
function change_balance(){
    ...
    data: {'login':slogin, 'balance':balance},
    });
}
...
function record_history(result_str){
  //  login=document.getElementById('name').innerText;
    $.ajax({
      ...
      data: {'login':slogin, 'bet':bet, 'result':result_str},
    });
}
...
function showPromt(promt){
    ...
    setTimeout(function(){el.css('opacity','0'); $('#bet-input').val("");},1500);
}
...
// suit of card
function getSuit(){
    return suits[Math.floor(Math.random()*4+1)];
}
